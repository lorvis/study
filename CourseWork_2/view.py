...
def generate_os_tree_html(root):
    ...
    for root, dirs, files in os.walk(root):
        ...
        if level == 0:
            dirname = 'Drive'
            roothash = saltyhash(root + os.sep)
...
def get_file_to_hashtree(root, hash, file, user_id):
    for root, dirs, files in os.walk(root):
        if hash == saltyhash(root):
            try:
                file.save(root + os.sep + file.filename)
            except:
                return Response(status=404)
            finally:
                User(user_id=session.get("ID"))["directories"] += 1
...
def get_treeobj_by_id(obj_id, user_id):
    ...
    for root, dirs, files in os.walk(base_root):
        if obj_id == saltyhash(root) or (root.replace(base_root, '') == '' and obj_id == saltyhash(root+os.sep)):
            root += os.sep
            return TreeObj(obj_id, os.path.dirname(root.replace(base_root, '')), root, None, "d")
        if root[len(root)-1] != os.sep:
            root += os.sep
        for f in files:
...
def tree_create_dir(obj_id, user_id, dirname):
    base_root = drive_root + str(user_id)
    for root, dirs, files in os.walk(base_root):
        if obj_id == saltyhash(root) or (root.replace(base_root, '') == '' and obj_id == saltyhash(root + os.sep)):
            try:
                os.mkdir(root+os.sep+dirname)
            except:
                pass
            finally:
                User(user_id=session.get("ID"))["directories"] += 1
...
def admincheck():
    ...
    if not user.exists() or user["type"] > acctypes.index("admin"):
        return False
    return True
...
def banned():
    user = User(user_id=session.get("ID"))
    if user.exists() and not user["active"]:
        return True
    return False
# endregion
...
def download(file_id):
    ...
    if result:
        User(user_id=session.get("ID"))["downloads"] += 1
        return result
    else:
...
def delete(obj_id):
    ...
    if user.exists():
        ...
        if os.path.isfile(path):
            User(user_id=session.get("ID"))["files"] -= 1
            os.remove(path)

        if os.path.isdir(path):
            files_count, dirs_count = tree_count_obj(path)
            User(user_id=session.get("ID"))["files"] -= files_count
            User(user_id=session.get("ID"))["directories"] -= dirs_count
            rmtree(path)
...
def upload(directory_id):
    ...
    if file.filename == '':
        return Response(status=400)
    get_file_to_hashtree(root, directory_id, file, session.get("ID"))
    return redirect(url_for('drive'))


@app.route('/drive/newdir/<dir_id>', methods=['POST'])
def newdir(dir_id):
    if not session.get("ID"):
        return Response(status=403)
    if request.form["dirname"] == '':
        return Response(status=204)
    tree_create_dir(dir_id, session.get("ID"), request.form["dirname"])
    return redirect(url_for('drive'))
...
def bancenter():
    if not admincheck():
        return Response(status=403)
    users = userlist()
    return render_template('bancenter.html', users=users)


@app.route('/bancenter/ban/<uid>')
def ban(uid):
    if not admincheck():
        return Response(status=403)
    user = User(user_id=int(uid))
    if user.exists():
        user["active"] = False
    return redirect(url_for('bancenter'))


@app.route('/bancenter/unban/<uid>')
def unban(uid):
    if not admincheck():
        return Response(status=403)
    user = User(user_id=int(uid))
