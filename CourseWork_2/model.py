...
class User:
    ...
    def gravatar(self, size):
        return 'http://www.gravatar.com/avatar/%s?d=mm&s=%d' % (md5(self.__body["email"].encode('utf-8')).hexdigest(), size)
...
def userlist():
    uidlist = list(get_storage_collection('users').find({}, {"ID": True}))
    return [User(user_id=uid["ID"]) for uid in uidlist]


def acctype_by_id(uid):
    coll = get_storage_collection("users")
    user = None
    try:
        user = coll.find_one({"ID": uid}, {"type": True})
    except:
        pass
    if user:
        return user["type"]
